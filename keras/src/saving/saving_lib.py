...
def _get_saveable_class_name(saveable):
    """Return the serialization name for a saveable.

    Walks up the MRO to find the first class defined in `keras.layers`
    (or `keras.src.layers`). This ensures that custom subclasses of
    built-in layers (e.g. `class MyLSTM(LSTM)`) serialize under the
    same name as the base class, making weight transfer between the
    subclass and the base class possible.
    """
    for cls in saveable.__class__.__mro__:
        mod = cls.__module__
        if mod and (
            mod.startswith("keras.layers") or mod.startswith("keras.src.layers")
        ):
...
def _save_container_state(
    ...
    for saveable in container:
        if isinstance(saveable, KerasSaveable):
            ...
            name = _get_saveable_class_name(saveable)
