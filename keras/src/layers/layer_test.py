...
class LayerTest(testing.TestCase):
    ...
    def test_symbolic_call_records_unconsumed_context_kwargs(self):
        # A `training=` passed to a layer whose `call()` does not declare it
        # is consumed as a call-context arg, but it must still be recorded on
        # the graph node so that replaying the functional graph re-establishes
        # the same context. Regression guard: `__call__` must not mutate the
        # `kwargs` dict it also hands to `Node` as `call_kwargs`.
        class Wrapper(layers.Layer):
            def __init__(self, **kwargs):
                super().__init__(**kwargs)
                self.dropout = layers.Dropout(1.0)
        ...
        inputs = Input((4,))
        wrapper = Wrapper()
        outputs = wrapper(inputs, training=True)
        self.assertEqual(
            wrapper._inbound_nodes[0].arguments.kwargs, {"training": True}
        )

        # The pinned `training=True` must survive graph replay: dropout with
        # rate 1.0 zeroes everything in training mode.
        model = models.Model(inputs, outputs)
        y = backend.convert_to_numpy(model(np.ones((2, 4), dtype="float32")))
        self.assertAllClose(y, np.zeros((2, 4), dtype="float32"))

    def test_symbolic_call_does_not_record_auto_populated_mask(self):
        # The auto-populated `mask=None` kwarg is an internal detail of
        # `__call__` and must not leak into the graph node's `call_kwargs`.
        class MaskArgLayer(layers.Layer):
        ...
        layer(inputs, training=True)
        self.assertEqual(
            layer._inbound_nodes[0].arguments.kwargs, {"training": True}
        )

    def test_set_mask_metadata_leaf_output_with_nested_compute_mask(self):
        # A `compute_mask` override that returns a structure for a single
        # tensor output takes the first flattened mask, and an empty
        # structure leaves the output unmasked.
        class NestedMask(layers.Layer):
        ...
        x = np.ones((2, 4), dtype="float32")
        first = backend.convert_to_tensor(np.ones((2,), dtype="bool"))
        second = backend.convert_to_tensor(np.zeros((2,), dtype="bool"))

        y = NestedMask([first, second])(x)
        self.assertIs(backend.get_keras_mask(y), first)

        y = NestedMask([])(x)
        self.assertIsNone(backend.get_keras_mask(y))
