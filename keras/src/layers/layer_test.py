...
class LayerTest(testing.TestCase):
    ...
    def test_call_spec_fast_path_matches_signature_bind(self):
        ...
        from keras.src.layers.layer import single_positional_call_defaults
        ...
        defaults = single_positional_call_defaults(signature)
        x, y = ops.ones((2, 3)), ops.zeros((2, 3))

        # Covered shapes: one positional argument, no keyword arguments
        # (`training` is stripped first when `call()` does not accept it).
        ...
        # Everything else falls back to `signature.bind`.
    ...
    def test_call_skips_signature_bind_for_single_input(self):
        ...
        dense(x), relu(x)  # Build outside the patch.
        ...
        with mock.patch.object(inspect.Signature, "bind", counting_bind):
            dense(x)
            relu(x, training=True)  # `training` is not in `ReLU.call`.
            self.assertEqual(len(bind_calls), 0)
            dense(x, training=True)  # `training` is in `Dense.call`.
